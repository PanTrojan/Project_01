...
public class PrismEffects : MonoBehaviour {
	...
	public BloomThreshholdType bloomThreshType = BloomThreshholdType.Gradual;
	
	public bool debugBloomTex = false;

	public BloomDownsampleAmount bloomDownsampleEnum = BloomDownsampleAmount.Quarter;

	[Range(1, 12)]
	public int bloomDownsample = 2;
	
	[Range(0, 12)]
	public int bloomBlurPasses = 1;
	...
	protected void SetBloomShaderValues (Material bloomMaterial)
	{
		...
		bloomMaterial.SetFloat("_DirtIntensity", diSqsuared);
	...
	MainPrismArea:
		...
		{		
			...
			if(raysShowDebug)
			{
				Graphics.Blit(quaterResSecond, destination);
		...
		if(useBloom) {		
			...
			if(bloomType == BloomType.Simple)
			{
				...
				if(useBloomFlares)
				{
					...
					RenderTexture blurOne = RenderTexture.GetTemporary (halfRezColorDown.width, halfRezColorDown.height, 0, halfRezColorDown.format);
					Graphics.Blit (featurePass, blurOne, m_Material2, 14);
					Graphics.Blit (blurOne, featurePass, m_Material2, 13);
					Graphics.Blit (featurePass, blurOne, m_Material2, 0);
					RenderTexture.ReleaseTemporary(featurePass);
					
					//Add
					m_Material.SetTexture("_FinalFlareTex", blurOne);
				}
				...
				if(useBloomFlares)
				{
					...
					//Add
					m_Material.SetTexture("_FinalFlareTex", featurePass);

					//RenderTexture.ReleaseTemporary(featurePass);

					if(debugBloomTex)
					{
						Graphics.Blit(featurePass, destination);
						goto ReleaseRenderTextureArea;
					}
				}
